...
Over time, we have employed machine learning to train a model to predict the target federal funds rate using FOMC texts. This blog will focuse on the two frameworks we used to train the model and the validation of their accuracy. 
...
Above all, we need to perform additional data processing to obtain a one-to-one feature matrix and target variable.

To clarify, we select the Upper Limit of the Target Fed Funds Rate every seven weeks as our target variable, since the Target Fed Funds Rate is typically published every seven weeks, and we use the Sentiment Analysis Scores from FinBERT to create the feature matrix.

For the feature matrix, we align the sentiment scores with the corresponding Fed Funds Rate dates, as shown in the following picture. In this representation, X denotes a list of sentiment scores, while Y represents the Fed Funds Rate.
...
Next, we pad the shorter lists in the DataFrame with zeros to match the maximum length. Then, we convert these padded lists into a numpy array for model input. Since zeros indicate neutrality, we believe this approach is acceptable. Finally, we obtain the feature matrix. The code we use is as follows:
```python
...
For the target variable, we classify Fed Funds Rate changes into three categories: Increase, Decrease, or No Change. The code we use is as follows:
```python
...
Our model is based on an LSTM neural network. We choose LSTM because our data exhibits temporal dependencies, and LSTM can capture both short-term and long-term trends. 

The neural network structure is as follows: The input layer takes the feature matrix, followed by two LSTM layers with dropout and L2 regularization to prevent overfitting. Then, the dense layer uses ReLU for non-linearity to enable the model to learn more complex patterns. Finally, the output layer uses a softmax activation function to convert the raw outputs into probabilities for each of the three categories.
...
After running the cross-validation, we calculate the average accuracy and F1-score across all folds. These metrics give us a reliable measure of the model's performance.
...
The results of the cross-validation, shown in the following picture, indicate an average accuracy of around 79% and an F1-score of approximately 70%. This suggests that our model performs fairly well, balancing precision and recall across the different classes.
